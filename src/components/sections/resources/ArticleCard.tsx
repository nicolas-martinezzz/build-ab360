    ...
    <article className="group flex h-full w-full flex-col overflow-hidden rounded-[10px] bg-white shadow-[var(--shadow-card)] transition hover:shadow-[var(--shadow-card-hover)]">
      <Link className="flex flex-1 flex-col" href={`/resources/${article.slug}` as never}>
        ...
        <div className="flex flex-1 flex-col px-5 pb-5 pt-4">
          <p className="figma-text-m leading-[1.4] text-grey-dark">
            <time dateTime={article.publishedAt}>{date}</time> · {article.readingTimeMin} {minReadLabel}
          </p>
          <h3 className="figma-card-title mt-2 text-surface-bg transition group-hover:text-green-600">
            {translation.title}
          </h3>
          <p className="figma-text-m mt-2 line-clamp-2 leading-relaxed text-surface-bg/70">
            {translation.excerpt}
          </p>
          ...
          <p className="figma-text-m mt-auto pt-3 font-medium text-green-600">
