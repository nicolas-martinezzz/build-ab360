...
const Benefit = ({ number, titlePrefix, titleSuffix, body }: BenefitProps) => (
  <li className="w-full max-w-[15.75rem]">
    <div className="flex flex-col gap-2">
      ...
      <p className="figma-text-l max-w-[14.25rem] text-grey-dark">{body}</p>
    </div>
  </li>
);
...
export const ProgramaEcosystemSection = async () => {
  ...
  return (
    <section className="bg-white py-16 md:py-20">
      <SectionContainer>
        <ul className="grid justify-items-start gap-x-8 gap-y-14 md:grid-cols-2 xl:grid-cols-4">
          {benefits.map((b) => (
